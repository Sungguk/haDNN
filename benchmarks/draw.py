...
def read():
    f_caffe = read_file('caffe-nopad-b64c64.data', 'Caffe')
    f_normal = read_file('normal-th1b64c64.data', 'Conv')
    f_fft = read_file('fft-th1b64c64.data', 'FFT')
    f_bin = read_file('bin-b64c64.data', 'Binary')
    datas = [f_caffe, f_normal, f_fft, f_bin]
    datas = [f_caffe, f_bin]
...
def print_data(data, args):
    ...
    for arg in args:
        if arg.startswith('c'):
            ...
            filtered.append(data['Binary k={}'.format(arg[1])])
        else:
            raise arg
